...
class CommentStudent {
    ...
    public function getTemplateCommentStudent() {
        ...
        <div class= "col-12 mb-4 p-0 form-floating w-100">
            <div class= "form-control comment-container h-auto position-relative" id = "<?= $this->idStudent . '-' . $this->educator->idUser ?>">
                ...
                <div class="d-flex justify-content-end ">
                    ...
                    <form action = "<?= $_SERVER["REQUEST_URI"] ?>" method="POST">
                        <input type = "hidden" name = "idStudent" value = "<?= $this->idStudent ?>">
                        <input type = "hidden" name = "idEducator" value = "<?= $this->educator->idUser ?>">
                        <input type = "hidden" name = "action" value = "deleteComment">
                        <button type = "submit" class = "btn btn-primary btn-delete-comment px-2">
            ...
            <label>
                <?= $this->educator->lastName ?> <?= $this->educator->firstName ?> --- <i class="bi bi-volume-up"></i>
            </label>
        </div>
        <?php
        return ob_get_clean();
